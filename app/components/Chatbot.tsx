...
const Chatbot: React.FC<ChatbotProps> = ({ onProductSelect, onProductSearch }) => {
    ...
    const handleProductSelect = (productId: number) => {
        ...
        setMessages(prev => [...prev, { text: `You selected product ID ${productId}`, sender: 'bot' }]);
        setIsOpen(false); // Optionally close the chat after selection
    };

    return (
        <>
            <div
                className="fixed bottom-5 right-5 bg-transparent text-white rounded-full w-16 h-16 p-2 flex items-center justify-center cursor-pointer shadow-lg transition-transform transform hover:scale-125"
                ...
                <div className={`fixed bottom-20 right-20 ${isExpanded ? 'w-[1000px] max-h-[600px]' : 'w-96 max-h-[500px]'} backdrop-filter backdrop-blur-sm bg-black bg-opacity-60 border border-gray-800 rounded-lg shadow-lg overflow-hidden transition-all duration-300`}>
                    ...
                    <div className="flex mt-auto p-2 border-t border-gray-700 ">
                        <input
                            ...
                            className="flex-grow border border-gray-300 rounded-lg p-2 text-black focus:outline-none focus:ring-gray-200"
                        />
                        <div
